 ...
 */
@interface NSString (CXDAdd)

//字符串判空
+ (BOOL)checkNullWithString:(NSString *_Nullable)tempString;
//空字符串时，文本设置为默认-
+ (NSString *_Nullable)stringLineNullString:(NSString *_Nullable)tempString;

//- (BOOL)isMobileNum;
+(BOOL)stringContainsEmoji:(NSString *_Nullable)string;

//检测密码是否是6-20位的字母与数字的组合
+ (BOOL)checkPassword:(NSString *_Nullable)password;

#pragma clang disanostic ignored "-Wincomplete-implementation"
